...
const Income = () => {
  ...
  const [formData, setFormData] = useState({ title: '', amount: '', category: 'Salary', description: '', date: '' });
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        console.error(err);
        toast.error('Unable to load incomes');
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleDelete = async (itemId) => {
    ...
    try {
      await api.delete(`/income/${itemId}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (selectedItem) {
        const { data } = await api.put(`/income/${selectedItem._id}`, {
          ...formData,
    ...
    <DashboardLayout>
      <div className="space-y-6">
        <div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
          <div>
            <p className="text-sm uppercase tracking-[0.3em] text-slate-400">Income Management</p>
            <h1 className="text-3xl font-semibold text-white">Income</h1>
          </div>
          <button
            onClick={openAdd}
            className="inline-flex items-center gap-2 rounded-2xl bg-indigo-500 px-5 py-3 text-sm font-semibold text-white shadow-lg shadow-indigo-500/20 hover:bg-indigo-400"
          >
            <FiPlus /> Add Income
          </button>
        </div>
        ...
        <div className="grid gap-4 xl:grid-cols-[1.25fr_0.75fr]">
          <div className="rounded-3xl border border-white/10 bg-slate-900/80 p-5 shadow-lg shadow-slate-950/40">
            <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
              <SearchBar value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search income" />
          ...
          <div className="rounded-3xl border border-white/10 bg-slate-900/80 p-5 shadow-lg shadow-slate-950/40">
            <p className="text-sm uppercase tracking-[0.3em] text-slate-400">Income total</p>
        ...
        {loading ? (
          ...
          <EmptyState title="No income found" subtitle="Add the first income transaction to begin tracking your cash flow." />
        )}
      </div>

      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={selectedItem ? 'Edit Income' : 'Add Income'}>
        <form ref={formRef} onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex justify-end gap-3 pt-2">
            ...
            <button type="submit" className="rounded-2xl bg-indigo-500 px-5 py-3 text-sm font-semibold text-white hover:bg-indigo-400 disabled:opacity-60" disabled={saving}>
              {saving ? 'Saving...' : selectedItem ? 'Update income' : 'Add income'}
            </button>
          </div>
        </form>
      </Modal>
